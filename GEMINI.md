...
- **Correções Recentes:**
    ...
    - **Leitura Direta da Instância Sem Perda de Payload & Correção de Task Runner:** Atualizado o nó de código no n8n para ler `EVOLUTION_INSTANCE` com verificações seguras de escopo (`(typeof $env !== 'undefined' && $env.EVOLUTION_INSTANCE) || (typeof process !== 'undefined' && process.env && process.env.EVOLUTION_INSTANCE)`), eliminando o erro `ReferenceError: process is not defined` no task runner JS (n8n v2.x sandbox).
    ...
    - **Sincronização Global de Credenciais do Trello no Dashboard:** Atualizados `docker-compose.yml`, `dashboard/env-injector.sh`, `dashboard/app.js` e `dashboard/index.html` para injetar `TRELLO_TOKEN` e `TRELLO_API_KEY` globalmente via `env.js`. O dashboard agora compartilha a sessão do Trello entre qualquer computador/navegador e permite salvar token/key manualmente (`saveManualTrelloCredentials`) ou realizar login OAuth limpando tokens legados.
    - **Parametrização 100% Universal via `.env`:** Adicionadas `TRELLO_API_KEY` e `TRELLO_TOKEN` a `N8N_ENV_VARS_ALLOW_ACCESS` em `docker-compose.yml` e parametrizadas todas as requisições HTTP do Trello via `$env`. Qualquer alteração de conta Trello/instância no `.env` reflete automaticamente em todos os fluxos e no dashboard sem edições de código.
    - **Suporte Multi-Instância e Tratamento de JIDs Privacy/LID (`@lid`):** Atualizados os parsers do n8n e o fluxo de importação para suportar instâncias individuais ou múltiplas separadas por vírgula em `EVOLUTION_INSTANCE`, além de tratar corretamente JIDs com formato `@lid` (WhatsApp Cloud API) para captura completa de nome e telefone.
---
